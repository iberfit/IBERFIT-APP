...
function mockResponse(action, payload) {
  ...
  if (action === "login") {
    const loginOk = String(payload.login || "").trim().toLowerCase() === IBERFIT_CONFIG.demoLogin;
    const passOk = String(payload.password || "").trim() === IBERFIT_CONFIG.demoPassword;

    if (!loginOk || !passOk) {
      return {
        ok: false,
        status: "MOCK_INVALID_LOGIN",
        message: "Modo mock: usa el usuario demo y la contraseña demo indicada para QA."
      };
    }
