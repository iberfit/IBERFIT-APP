/* IBERFIT V9.3 · App Coach OS · Production Candidate
...
function mockExercises() {
  return [
    { id: "EJ-0001", name: "Sentadilla goblet", pattern: "Sentadilla", zone: "Tren inferior", material: "Kettlebell/mancuerna", level: "Base", objective: "Fuerza técnica", cues: "Torso firme, rodillas acompañan línea del pie.", errors: "Colapsar rodillas, perder profundidad activa.", regression: "Sentadilla a cajón", progression: "Sentadilla frontal", mediaStatus: "Pendiente foto propia" },
    { id: "EJ-0002", name: "Peso muerto rumano", pattern: "Bisagra", zone: "Cadena posterior", material: "Mancuernas/barra", level: "Base", objective: "Fuerza posterior", cues: "Bisagra de cadera, columna neutra, recorrido controlado.", errors: "Flexionar lumbar, alejar carga.", regression: "Bisagra con pared", progression: "RDL barra", mediaStatus: "Pendiente foto propia" },
    { id: "EJ-0003", name: "Press mancuernas inclinado", pattern: "Empuje", zone: "Torso", material: "Mancuernas/banco", level: "Intermedio", objective: "Fuerza tren superior", cues: "Escápulas estables, control de bajada.", errors: "Hombros elevados, arco excesivo.", regression: "Press en suelo", progression: "Press barra", mediaStatus: "Pendiente" },
    { id: "EJ-0004", name: "Remo mancuerna", pattern: "Tracción", zone: "Torso", material: "Mancuerna", level: "Base", objective: "Tracción horizontal", cues: "Tirar con codo, controlar tronco.", errors: "Rotar tronco, encoger hombro.", regression: "Remo banda", progression: "Remo pecho apoyado", mediaStatus: "Pendiente" },
    { id: "EJ-0005", name: "Pallof press", pattern: "Anti-rotación", zone: "Core", material: "Banda/polea", level: "Base", objective: "Control lumbo-pélvico", cues: "Pelvis estable, exhalación al extender.", errors: "Rotar tronco, bloquear respiración.", regression: "Isométrico cerca del anclaje", progression: "Pallof caminata", mediaStatus: "Pendiente" }
  ];
}
...
function shell() {
  ...
  return `<section class="coach-app v9">
    <aside class="coach-sidebar">
      <div class="sidebar-brand coach-side-brand"><div class="sidebar-iso"><img src="/assets/iberfit-isotipo.png" alt="IBERFIT"></div><strong>IBERFIT</strong><span>Coach OS V9</span></div>
      <nav class="coach-tabs">
        ${tabBtn("panel", "Hoy")}
        ${tabBtn("clientes", "Clientes")}
        ${tabBtn("ficha", "Ficha")}
        ${tabBtn("sesion", "Presencial")}
      ...
      <p class="coach-mini">Cliente activo:<br><strong>${html(clientName)}</strong><br><span>${html(estado.espacio?.client?.modality || "")}</span></p>
    </aside>
    <main class="coach-main">
      <header class="coach-topbar"><div><span class="eyebrow">IBERFIT · Sistema interno</span><h1>${tituloVista()}</h1></div><div class="coach-top-actions"><button class="btn btn-ghost" id="btnGoClientes">Elegir cliente</button><button class="btn btn-primary" id="btnNuevoCliente">Crear cliente</button></div></header>
      ${avisoHtml()}
      ${vistaActual()}
    </main>
  </section>`;
...
function vistaInformes() {
  ...
  return `<section class="coach-grid"><div class="coach-card wide"><h2>Informes adjuntos a ficha</h2><div class="report-list">${reports.map(r => `<article class="report-card"><span>${html(r.type)} · ${fecha(r.date)}</span><h3>${html(r.title)}</h3><p>${html(r.internalSummary || "")}</p><small>${html(r.state)} · visible cliente: ${html(r.clientVisible)}</small></article>`).join("") || empty("Sin informes generados.")}</div></div><div class="coach-card wide"><h2>Nuevo informe</h2>${formInforme()}</div><div class="coach-card wide"><h2>Gráficas útiles</h2>${graficaCargas(estado.espacio.loadHistory || [])}</div></section>`;
}
function formInforme() {
  return `<form id="reportForm" class="stack-form"><div class="form-row"><label>Tipo<select id="reportType"><option>IRI</option><option>Sesión</option><option>Progreso</option><option>Mensual</option><option>Cargas</option></select></label><label>Estado<select id="reportState"><option>INTERNO</option><option>BORRADOR</option><option>PUBLICADO</option></select></label></div><label>Título<input id="reportTitle" placeholder="Informe IBERFIT"></label><label>Versión interna coach<textarea id="reportInternal" rows="5" placeholder="Más técnica, densa, con decisiones"></textarea></label><label>Versión cliente<textarea id="reportClient" rows="5" placeholder="Clara, sobria y entendible"></textarea></label><label>URL PDF / Drive<input id="reportUrl" placeholder="Opcional"></label><button class="btn btn-primary" type="submit">Guardar informe en ficha</button></form>`;
}
...
function vistaBiblioteca() {
  const library = safeArray(estado.panel?.exerciseLibrary || mockExercises());
  const filtro = estado.filtroEjercicio.toLowerCase();
