/* IBERFIT V9.4.1 · Mobile Login Reliability Fix + Coach & Client Experience Upgrade
   V9.6.2 Launch Safe: sesión IA + anti-pantalla-vacía + fallback visual. */
...
function renderFatal_(err, fase = "renderizado") {
  const msg = err && (err.message || String(err)) || "Error desconocido";
  const file = err && (err.filename || err.fileName || err.stack || "");
  console.error("IBERFIT Coach OS", fase, err);
  if (root) {
    root.innerHTML = `<main class="coach-login-screen"><section class="login-hero"><div class="brand-mark">IBERFIT</div><h1>Coach OS</h1><p>La interfaz no pudo completar el ${html(fase)}.</p><div class="coach-card" style="max-width:720px"><h2>Modo seguro IBERFIT</h2><p>Esto evita la pantalla verde vacía. Recarga forzada y verifica conexión. Si persiste, vuelve a subir el paquete V9.6.2 Launch Safe completo.</p><pre style="white-space:pre-wrap;background:rgba(0,0,0,.18);padding:12px;border-radius:12px">${html(msg)}</pre><button class="btn btn-primary" onclick="location.reload()">Recargar</button></div></section></main>`;
  }
}
window.addEventListener("error", (event) => renderFatal_(event.error || event.message, "carga"));
window.addEventListener("unhandledrejection", (event) => renderFatal_(event.reason, "promesa"));
const COACH_KEY = "IBERFIT_COACH_SESSION_V9_4";
const COACH_LEGACY_KEYS = ["IBERFIT_COACH_SESSION_V9_3"];
...
function removeCoachSession() {
  [COACH_KEY, ...COACH_LEGACY_KEYS].forEach(storageRemove);
}
function leerSesion() {
  for (const key of [COACH_KEY, ...COACH_LEGACY_KEYS]) {
    try {
      const raw = storageRead(key);
      if (!raw) continue;
      const s = JSON.parse(raw);
      if (!s.coachToken) continue;
      if (s.expiresAt && Date.now() > new Date(s.expiresAt).getTime()) {
        storageRemove(key);
        continue;
      }
      if (key !== COACH_KEY) storageWrite(COACH_KEY, JSON.stringify(s));
      return s;
    } catch {
      storageRemove(key);
    }
...
function guardarSesionCoach(data) {
  ...
  storageWrite(COACH_KEY, JSON.stringify(s));
...
function normalizeApiResponse(data) {
  ...
  const isCloudflareEnvelope =
    ...
    data.service.includes("IBERFIT Cloudflare API") &&
    data.data &&
    typeof data.data === "object";
  if (!isCloudflareEnvelope) return data;
  const upstream = data.data;
  if (data.ok === false && upstream.ok !== false) {
    return {
      ok: false,
      status: data.error || data.status || "API_PROXY_ERROR",
      message: data.message || data.error || "No pudimos completar la solicitud.",
      data: upstream
    };
  }
  return upstream;
...
async function api(action, payload = {}) {
  ...
  try {
    ...
    try { return normalizeApiResponse(JSON.parse(text)); }
...
function mock(action, payload) {
  ...
  if (action === "coachAiGenerate") return { ok: true, data: { provider: "LOCAL", fallback: true, fields: mockAiFields(payload.mode || "REPORT") } };
...
function mockAiFields(mode) {
  const m = String(mode || "REPORT").toUpperCase();
  const common = {
    title: "Informe IBERFIT · cliente",
    clientVersion: "Hoy priorizamos continuidad, técnica y una progresión ajustada a tu respuesta real.",
    internalVersion: "IA local IBERFIT: revisar IRI, cargas, feedback y check-in antes de progresar.",
    decision: "Progresar con prudencia: no subir carga y volumen a la vez.",
    nextAction: "Revisar respuesta real y ajustar próxima sesión."
  };
  if (m === "SESSION") return { ...common, title: "Sesión IBERFIT · fuerza controlada", objective: "Consolidar técnica y adherencia.", criterion: "Hoy priorizamos técnica limpia, RPE 6–8 y control de molestias.", indications: "Registrar RPE, energía y molestia al cierre.", observe: "Técnica, RPE y tolerancia.", adjust: "Modificar solo una variable por vez.", report: "Reportar dificultad y molestia." };
  if (m === "CLOSE_SESSION") return { ...common, internalSummary: common.internalVersion, clientSummary: common.clientVersion };
  return common;
}
...
function loginPantalla() {
  return `<main class="coach-login-screen">
    ...
    <form class="coach-login-card" id="coachLoginForm">
      ...
      <label>Usuario<input id="coachLogin" type="email" inputmode="email" autocomplete="username" autocapitalize="none" autocorrect="off" spellcheck="false" placeholder="iberfit.cl@gmail.com"></label>
      <label>Contraseña<input id="coachPassword" type="password" autocomplete="current-password" autocapitalize="none" autocorrect="off" spellcheck="false" placeholder="Contraseña"></label>
      <button id="coachLoginSubmit" class="btn btn-primary btn-full" type="button">Entrar</button>
      ${avisoHtml()}
...
function vistaPanel() {
  ...
  return `<section class="coach-grid v9-dashboard">
    ...
    <div class="coach-card wide"><div class="section-head"><div><h2>Motor de Decisión IBERFIT</h2><p>Lectura automática basada en IRI, cargas, feedback, check-in y alertas.</p></div><span class="badge-soft gold">Gemini + IA local</span></div>${decisionEnginePanel(p)}</div>
  </section>`;
}
function metricas(m = {}) {
  const items = [
...
function formConstructorSesion(e) {
  ...
  return `<form id="sessionBuilderForm" class="stack-form builder-form">
    <div class="builder-steps"><span>1 · Objetivo</span><span>2 · IA propone sesión completa</span><span>3 · Carlos edita</span><span>4 · Publicar</span></div><div class="ai-action-row ai-action-row-strong"><button class="btn btn-primary" id="aiFullSessionBtn" type="button">Crear sesión entera con IA</button><button class="btn btn-ghost" id="aiSessionBtn" type="button">Mejorar criterio</button><button class="btn btn-ghost" id="aiReviewSessionBtn" type="button">Revisar sesión con IA</button><small>Gemini si está configurado; IA local si falla. Nada se publica sin aprobación de Carlos.</small></div>
    ...
    <div class="builder-structure-grid">
      ...
      ${builderBlockInputs(3, "Vuelta a la calma", "Respiración + movilidad + registro", "", "4-6 min", "", "Cerrar con feedback")}
    </div>
...
async function generarIaIberfit(mode, extra = {}) {
  if (!estado.clienteId) { aviso("Primero elige cliente.", "error"); return null; }
  aviso("Generando con IA IBERFIT...");
  const res = await api("coachAiGenerate", { clientId: estado.clienteId, mode, ...extra });
  if (!res.ok) { aviso(res.message || "No se pudo generar texto IA.", "error"); return null; }
  const data = res.data || {};
  const provider = data.provider === "GEMINI" ? "Gemini" : "IA local";
  aviso(`Texto generado con ${provider}. ${data.warning || ""}`, data.provider === "GEMINI" ? "success" : "info");
  return data.fields || {};
}
function setValueIfPresent(selector, value, overwrite = true) {
  const el = $(selector);
  if (!el || value === undefined || value === null) return;
  if (!overwrite && String(el.value || "").trim()) return;
  el.value = String(value || "");
}
async function aplicarIaSesion() {
  const fields = await generarIaIberfit("SESSION", {
    title: $("#builderTitle")?.value || "",
    objective: $("#builderObjective")?.value || "",
    criterion: $("#builderCriterion")?.value || "",
    desiredDuration: $("#builderDuration")?.value || "55",
    material: $("#builderMaterial")?.value || "",
    deliveryMode: $("#builderDelivery")?.value || "PRESENCIAL",
    sessionDraft: JSON.stringify(leerBloquesConstructor())
  });
  if (!fields) return;
  setValueIfPresent("#builderTitle", fields.title, false);
  setValueIfPresent("#builderObjective", fields.objective);
  setValueIfPresent("#builderCriterion", fields.criterion);
  setValueIfPresent("#builderIndications", fields.indications || fields.clientVersion);
  setValueIfPresent("#builderMaterial", fields.material, false);
}
async function aplicarIaSesionCompleta() {
  const fields = await generarIaIberfit("SESSION_FULL", {
    title: $("#builderTitle")?.value || "",
    objective: $("#builderObjective")?.value || "",
    criterion: $("#builderCriterion")?.value || "",
    desiredDuration: $("#builderDuration")?.value || "55",
    material: $("#builderMaterial")?.value || "",
    deliveryMode: $("#builderDelivery")?.value || "PRESENCIAL",
    currentSessionJson: JSON.stringify(leerBloquesConstructor())
  });
  if (!fields) return;
  aplicarSesionEstructurada(fields);
}
async function revisarSesionConIa() {
  const fields = await generarIaIberfit("SESSION_REVIEW", {
    title: $("#builderTitle")?.value || "",
    objective: $("#builderObjective")?.value || "",
    criterion: $("#builderCriterion")?.value || "",
    currentSessionJson: JSON.stringify(leerBloquesConstructor())
  ...
  if (!fields) return;
  const audit = Array.isArray(fields.audit) ? fields.audit.join("\n- ") : (fields.internalVersion || fields.decision || "");
  const current = $("#builderCriterion")?.value || "";
  setValueIfPresent("#builderCriterion", `${current}\n\nRevisión IA IBERFIT:\n- ${audit}`.trim());
  setValueIfPresent("#builderIndications", fields.clientVersion || fields.indications, false);
}
function aplicarSesionEstructurada(fields) {
  const session = fields.session && typeof fields.session === "object" ? fields.session : fields;
  setValueIfPresent("#builderTitle", session.title || fields.title, true);
  setValueIfPresent("#builderObjective", session.objective || fields.objective, true);
  setValueIfPresent("#builderCriterion", session.criterion || fields.criterion || fields.internalVersion, true);
  setValueIfPresent("#builderIndications", session.indications || fields.indications || fields.clientVersion, true);
  setValueIfPresent("#builderMaterial", session.material || fields.material, true);
  setValueIfPresent("#builderDuration", session.duration || fields.duration, true);
  if (session.deliveryMode) setValueIfPresent("#builderDelivery", session.deliveryMode, true);
  if (session.type) setValueIfPresent("#builderType", session.type, true);
  const blocks = normalizarBloquesIa(session.blocks || fields.blocks || []);
  blocks.slice(0,4).forEach((b, i) => {
    setValueIfPresent(`#builderBlock${i}Title`, b.title || `Bloque ${i+1}`, true);
    setValueIfPresent(`#builderBlock${i}Focus`, b.focus || "", true);
    safeArray(b.exercises).slice(0,2).forEach((ex, j) => {
      setValueIfPresent(`#builderB${i}E${j}Name`, ex.name || ex.exercise || "", true);
      setValueIfPresent(`#builderB${i}E${j}Sets`, ex.sets || "", true);
      setValueIfPresent(`#builderB${i}E${j}Reps`, ex.reps || ex.time || "", true);
      setValueIfPresent(`#builderB${i}E${j}Rest`, ex.rest || "", true);
      setValueIfPresent(`#builderB${i}E${j}Note`, [ex.load, ex.rpe ? `RPE ${ex.rpe}` : "", ex.note, ex.regression ? `Regresión: ${ex.regression}` : "", ex.progression ? `Progresión: ${ex.progression}` : ""].filter(Boolean).join(" · "), true);
    });
  });
  aviso("Sesión completa creada como borrador editable. Revisa, cambia ejercicios y guarda cuando esté aprobada.", "success");
}
function normalizarBloquesIa(blocks) {
  if (!Array.isArray(blocks)) return [];
  return blocks.map(b => ({ title: b.title || b.nombre || "Bloque", focus: b.focus || b.objetivo || "", exercises: safeArray(b.exercises || b.ejercicios).map(ex => ({ name: ex.name || ex.ejercicio || ex.exercise || "", sets: ex.sets || ex.series || "", reps: ex.reps || ex.repeticiones || ex.time || ex.tiempo || "", rest: ex.rest || ex.descanso || "", load: ex.load || ex.carga || "", rpe: ex.rpe || ex.rpeObjetivo || "", note: ex.note || ex.nota || ex.indicaciones || "", regression: ex.regression || ex.regresion || "", progression: ex.progression || ex.progresion || "" })) }));
}
async function aplicarIaInforme() {
  const fields = await generarIaIberfit("REPORT", { type: $("#reportType")?.value || "", audience: $("#reportAudience")?.value || "", period: $("#reportPeriod")?.value || "" });
  if (!fields) return;
  setValueIfPresent("#reportTitle", fields.title, false);
  setValueIfPresent("#reportInternal", fields.internalVersion);
  setValueIfPresent("#reportClient", fields.clientVersion);
}
async function aplicarIaIri() {
  const fields = await generarIaIberfit("IRI", { currentText: [$("#iriContext")?.value, $("#iriLimiters")?.value, $("#iriRecommendation")?.value, $("#iriDecision")?.value].join("\n") });
  if (!fields) return;
  setValueIfPresent("#iriRecommendation", fields.recommendation || fields.clientVersion);
  setValueIfPresent("#iriDecision", fields.decision || fields.internalVersion);
}
async function aplicarIaCierre() {
  const fields = await generarIaIberfit("CLOSE_SESSION", { sessionDraft: resumenDrafts(estado.borradorPresencial[estado.sesionActiva] || []) });
  if (!fields) return;
  setValueIfPresent("#closeInternal", fields.internalSummary || fields.internalVersion);
  setValueIfPresent("#closeClient", fields.clientSummary || fields.clientVersion);
  setValueIfPresent("#closeDecision", fields.decision || fields.nextAction);
}
async function aplicarIaDecision(tipo, id) {
  const fields = await generarIaIberfit(tipo === "feedback" ? "FEEDBACK" : "CHECKIN", { feedbackId: tipo === "feedback" ? id : "", checkinId: tipo === "checkin" ? id : "" });
  if (!fields) return;
  const input = document.querySelector(`[data-decision-text="${tipo}:${id}"]`);
  if (input) input.value = fields.decision || fields.nextAction || fields.internalVersion || "";
}
async function reemplazarEjercicioConIa(name) {
  const reason = prompt("Motivo del reemplazo IBERFIT", "molestia, fatiga, técnica baja o falta de material") || "ajuste en vivo";
  const fields = await generarIaIberfit("EXERCISE_REPLACE", { exerciseName: name, replaceReason: reason, currentSessionJson: JSON.stringify(leerBloquesConstructor()) });
  if (!fields) return;
  const reps = safeArray(fields.replacements);
  const msg = reps.length ? reps.map((r, i) => `${i+1}. ${r.name || r.exercise || "Alternativa"} — ${r.reason || r.cues || "misma intención"}`).join("
") : (fields.internalVersion || fields.decision || "Sin alternativa");
  aviso(`Alternativas IA para ${name}:
${msg}`, "info");
  const first = reps[0];
  if (first && first.name && confirm(`Usar primera alternativa: ${first.name}?`)) {
    const input = $("#loadExercise");
    if (input) input.value = first.name;
  }
}
async function adaptarEnVivoConIa(name) {
  const liveIssue = prompt("Qué ocurre en la sesión", "RPE alto / molestia / falta de tiempo / técnica mala / fatiga") || "ajuste en vivo";
  const fields = await generarIaIberfit("LIVE_ADAPTATION", { exerciseName: name, liveIssue, currentSessionJson: JSON.stringify(leerBloquesConstructor()) });
  if (!fields) return;
  aviso(fields.decision || fields.internalVersion || fields.clientVersion || "Ajuste generado.", "info");
  const notes = $("#loadNotes");
  if (notes) notes.value = `${notes.value || ""}
IA IBERFIT (${name}): ${fields.decision || fields.internalVersion || "ajuste recomendado"}`.trim();
}
...
function liveExerciseCard(ex, sessionId) {
  ...
  return `<article class="exercise-live-card"><div><span>${html(ex.block || "Bloque")}</span><h3>${html(ex.name)}</h3><p>${html(ex.note || ex.focus || "")}</p></div><div class="exercise-prescription"><b>${html(ex.sets || "—")} x ${html(ex.reps || ex.time || "—")}</b><small>${html(ex.load || "Carga según histórico")} · RPE ${html(ex.rpe || "objetivo")}</small></div><div class="recommendation-box"><strong>${html(rec.title)}</strong><small>${html(rec.text)}</small></div><div class="live-actions"><button class="btn btn-ghost btn-small" data-register-exercise="${html(ex.name)}" data-session-id="${html(sessionId)}">Registrar</button><button class="btn btn-ghost btn-small" data-ai-replace-exercise="${html(ex.name)}" type="button">Reemplazar IA</button><button class="btn btn-ghost btn-small" data-ai-live-adapt="${html(ex.name)}" type="button">Ajustar IA</button></div></article>`;
}
...
function formCierreSesion(session, drafts) {
  return `<form id="closeSessionForm" class="stack-form"><label>Resumen interno coach<textarea id="closeInternal" rows="4" placeholder="Qué ocurrió, cambios, técnica, cargas, alertas">${html(resumenDrafts(drafts))}</textarea></label><label>Resumen cliente<textarea id="closeClient" rows="3" placeholder="Resumen claro para cliente"></textarea></label><label>Decisión próxima<textarea id="closeDecision" rows="3" placeholder="Progresar, mantener, ajustar, reevaluar"></textarea></label><div class="ai-action-row"><button class="btn btn-ghost" id="aiCloseBtn" type="button">Generar resumen con IA IBERFIT</button><small>Usa borrador de cargas, cliente e histórico.</small></div><button class="btn btn-primary" type="submit">Cerrar sesión + generar resumen</button></form>`;
}
...
function bind() {
  ...
  $("#coachLoginSubmit")?.addEventListener("click", entrar);
  $("#coachRefreshBtn")?.addEventListener("click", actualizar);
  $("#coachLogoutBtn")?.addEventListener("click", async () => { try { await api("coachLogout"); } catch {} removeCoachSession(); estado.token = ""; estado.panel = null; estado.espacio = null; render(); });
  ...
  $("#aiFullSessionBtn")?.addEventListener("click", aplicarIaSesionCompleta);
  $("#aiSessionBtn")?.addEventListener("click", aplicarIaSesion);
  $("#aiReviewSessionBtn")?.addEventListener("click", revisarSesionConIa);
  ...
  $("#aiIriBtn")?.addEventListener("click", aplicarIaIri);
  $("#reportForm")?.addEventListener("submit", guardarInforme);
  $("#aiReportBtn")?.addEventListener("click", aplicarIaInforme);
  ...
  $("#aiCloseBtn")?.addEventListener("click", aplicarIaCierre);
  ...
  $$('[data-ai-replace-exercise]').forEach(b => b.addEventListener("click", () => reemplazarEjercicioConIa(b.dataset.aiReplaceExercise)));
  $$('[data-ai-live-adapt]').forEach(b => b.addEventListener("click", () => adaptarEnVivoConIa(b.dataset.aiLiveAdapt)));
  ...
  $$('[data-ai-decision]').forEach(b => b.addEventListener("click", () => { const [tipo, id] = String(b.dataset.aiDecision || "").split(":"); aplicarIaDecision(tipo, id); }));
...
function render() {
  try {
    if (!root) throw new Error("No existe #coachRoot en coach.html");
    root.innerHTML = estado.token ? shell() : loginPantalla();
    bind();
  } catch (err) {
    renderFatal_(err, "renderizado");
  }
}
async function init() {
  try {
    ...
    if (s) {
      ...
      estado.coach = s.coach || {};
      const res = await api("coachGetDashboard");
      if (res.ok) estado.panel = res.data; else { removeCoachSession(); estado.token = ""; }
    }
    render();
  } catch (err) {
    removeCoachSession();
    estado.token = "";
    renderFatal_(err, "inicio");
  }
}
init();
